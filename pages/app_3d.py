...
# ---------------- Default Clouds ----------------
default_A = pd.DataFrame({"L": ["A1","A2","A3"], "X":[0,1,2], "Y":[0,1,0.5], "Z":[0,1,0.5] })
default_B = pd.DataFrame({"L": ["B1","B2","B3"], "X":[1,2,3], "Y":[2,1.5,1], "Z":[2,1.5,1]})
...
for col, file, cloud_name, key_name in [(col_up1, file_A, "cloud_A", "cloud_A_editor_key"),
    ...
    if col.button(f"Load CSV → Table {cloud_name[-1]}"):
        if file is None:
            ...
            try:
                df = pd.read_csv(file)
                if all(c in df.columns for c in ["L","X","Y","Z"]):
                    ...
                    st.error("CSV must contain columns: L, X, Y, Z")
...
if generate_clicked:
    ...
    st.session_state["cloud_B"] = pd.DataFrame({"L":[f"B{i}" for i in range(len(Y))], "X":Y[:,0], "Y":Y[:,1], "Z":Y[:,2]})
...
# ---------------- Button Actions ----------------
if plot_clicked:
    plot_clouds_3d(A["X"], A["Y"], A["Z"], B["X"], B["Y"], B["Z"], A["L"], B["L"], plot_placeholder)

if center_clicked:
    XA_orig, YA_orig, ZA_orig = A["X"].copy(), A["Y"].copy(), A["Z"].copy()
    XB_orig, YB_orig, ZB_orig = B["X"].copy(), B["Y"].copy(), B["Z"].copy()
    cxA, cyA, czA = XA_orig.mean(), YA_orig.mean(), ZA_orig.mean()
    cxB, cyB, czB = XB_orig.mean(), YB_orig.mean(), YB_orig.mean()
    frames_anim = 30
    for i in range(frames_anim):
        t = (i+1)/frames_anim
        plot_clouds_3d(XA_orig - t*cxA, YA_orig - t*cyA,ZA_orig - t*czA  , XB_orig - t*cxB, YB_orig - t*cyB,  ZB_orig + t*czB , A["L"], B["L"], plot_placeholder)
        time.sleep(0.02)

if compute_clicked:
    A_np = st.session_state["cloud_A"][["X","Y","Z"]].values
    B_np = st.session_state["cloud_B"][["X","Y","Z"]].values
    ...
    if A_np.shape[0]<1 or B_np.shape[0]<1:
        ...
        with st.spinner("Computing isometry…"):
            try:
                out = compute_isometry_nD(A_np, B_np, maxchunksize, sinkhorn_iters, rot_iters, reg)
            except Exception as e:
                st.exception(f"Compute failed: {e}")
                raise

        # --- Build animation frames ---
        frames = build_animation_frames_3d(A_np, B_np, out["A2"], out["B2"], out["R"], A["L"], B["L"])
...
if show_conn_clicked:
    if st.session_state.get("compute_output") is None:
        ...
        plot_clouds_with_matches_3d(
            A_final[:,0], A_final[:,1],A_final[:,2],
            B2[:,0], B2[:,1],B2[:,2],
